...
* The preconditioner configuration does not work as-is in NekRS so that was commented out in the .par file. Not sure how to port that.
...
* It assumed Fr as nu^2/(gL), not nu/sqrt(gL) but specified the Fr value (correctly) as nu/sqrt(gL). So it applies too much gravity.
